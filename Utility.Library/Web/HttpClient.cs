...
namespace Common.Library.Web
{
    ...
    public static class HttpClient
    {
        ...
        public static string DoPost(PostData post)
        {
            System.GC.Collect();
            ...
            try
            {
                ...
                * ************************************************************/
                ...
                byte[] data = System.Text.Encoding.UTF8.GetBytes(post.Json);
        ...
        public static string DoGet(PostData data)
        {
            System.GC.Collect();
            string result = "";
            ...
            try
            {
                ...
                ServicePointManager.DefaultConnectionLimit = 200;
                //设置https验证方式
                if (data.Url.StartsWith("https", StringComparison.OrdinalIgnoreCase))
                {
                    ServicePointManager.ServerCertificateValidationCallback =
                            new RemoteCertificateValidationCallback(CheckValidationResult);
                }
                ...
                * 下面设置HttpWebRequest的相关属性
                * ************************************************************/
                request = (HttpWebRequest)WebRequest.Create(data.ToUrl());
                request.UserAgent = "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1; .NET CLR 2.0.50727; .NET CLR 3.0.04506.648; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729)";
                request.Method = "GET";
                request.Timeout = data.Timeout * 1000;
                //获取服务器返回
                response = (HttpWebResponse)request.GetResponse();
    ...
    public class PostData
    {
        ...
        public string ToUrl()
        {
            var value = string.Join("&", string.Join("&", Params.Select(s => string.Format("{0}={1}", s.Key, s.Value.ToString()))));
